...
public class ProjectTaskSelectMembersAssignedClass {
 ...
 private ArrayList<String>  chatMemberArrayList;
    ...
    private ArrayList<JSONObject> notificationMemberJArray;
    ...
    public ProjectTaskSelectMembersAssignedClass( LinearLayout selectedFriendHViewLinearLayout, Button selectedFriendsButton,final Activity activity,String result)
    {
        ...
        try {
            this.memberJSONArray = new JSONArray(result);
            if(result!=null) {
                this.memberJSONArray = new JSONArray(result);
            }
            else
    ...
    public void sendNotificationsToUsers(String message,String requestType)
    {
        ...
        if(result.equalsIgnoreCase("Record Not Inserted"))
        {
            Toast.makeText(activity,"It seems that we cannot send notification to user now. Please unassign and reassign this task to members",Toast.LENGTH_LONG).show();
                ...
                for (int i = 0; i < notificationMemberJArray.size(); i++) {

                    try {
                        result = getOneSignalIdAttachedUserName(((JSONObject)notificationMemberJArray.get(i)).getString("userName"));
    ...
    public void membersListVwNormalScrnSizeLT() {
        ...
        try {
            ...
            selectedMemberArrayList = new ArrayList<String>();
            notificationMemberJArray = new ArrayList<JSONObject>();
                    ...
                    allFunctionCheckBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                        ...
                        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                            try {
                                ...
                                JSONObject tempObject = new JSONObject(memberJSONArray.getString(index));

                                if (isChecked) {
                                    selectedFriendsButton.setEnabled(true);
                                    seletedMemberJSONArray.put(tempObject.getString("userName"));
                                    chatMemberArrayList.add(tempObject.getString("userName"));
                                    selectedMemberArrayList.add(tempObject.getString("userName"));
                                    JSONObject notificationObject = new JSONObject();
                                    notificationObject.put("userName",tempObject.getString("userName"));
                                    notificationObject.put("groupName",groupName);
                                    notificationMemberJArray.add(notificationObject);

                                } else {
                                    if (selectedMemberArrayList.size() == 0) {
                                        selectedFriendsButton.setEnabled(false);
                                    }
                                    selectedMemberArrayList.remove(tempObject.getString("userName"));
                                    seletedMemberJSONArray.remove(index);
                                    chatMemberArrayList.remove(tempObject.getString("userName"));
                                    JSONObject notificationObject = new JSONObject();
                                    notificationObject.put("userName",tempObject.getString("userName"));
                                    notificationObject.put("groupName",groupName);
                                    notificationMemberJArray.add(notificationObject);
...

    {
       ...
       String taskAssignedDate  = new SimpleDateFormat("yyyy/MM/dd", Locale.getDefault()).format(new Date());
        ...
        try {
            ...
            httpRequestClass = new HttpRequestClass("http://naijaconnectsapis.ca/projectmanagement-1.0/projectmanagement/duftadg/",postDataParams, activity,"text/plain", "application/json");
            result = httpRequestClass.getResult();
            asyncErrorDialogDisplay = new AsyncErrorDialogDisplay(activity);
            if(result!=null) {
                if (result.equalsIgnoreCase("exception")) {
