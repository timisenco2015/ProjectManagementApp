...
public class ProjectGroupAddMemberClass {
    ...
    private JSONArray notGroupMemberJSONArray;
...
private String firstName, lastName;
   ...
   private boolean isDefaultGrp;
    ...
    public ProjectGroupAddMemberClass(final Activity activity,final String projectName, String userName,String firstName, String lastName,final AutoCompleteTextView groupMbrsAddAutoCompleteTextView,final LinearLayout addFriendsToGrpLinearLayout,final LinearLayout addGroupMembersBtnLinearlayout, RadioGroup groupAddSwitchRadioGroup,String defaultGroupName)
    {
...
this.defaultGroupName=defaultGroupName;
        isDefaultGrp = true;
        this.groupAddSwitchRadioGroup = groupAddSwitchRadioGroup;
       this.groupMbrsAddAutoCompleteTextView = groupMbrsAddAutoCompleteTextView;
        this.addFriendsToGrpLinearLayout = addFriendsToGrpLinearLayout;
        this.addGroupMembersBtnLinearlayout = addGroupMembersBtnLinearlayout;
        this.s3ImageClass = new S3ImageClass();
        zihronChatAppGroupChannel = new ZWCGroupChannel(activity);
        addGroupMembersBtnLinearlayout.setEnabled(false);

        try {

            String result = null;

            result = getAllProjectGroups(projectName,defaultGroupName);
            ...
            groupAddSwitchRadioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(RadioGroup group, int checkedId) {
                    ...
                    if(checkedId == R.id.groupCreatedAddRadioBtnId)
                    {
                        isDefaultGrp = false;
                        addFriendsToGrpLinearLayout.removeAllViews();
                        ...
                        groupMbrsAddAutoCompleteTextView.setEnabled(true);
                        groupMbrsAddAutoCompleteTextView.setText("");
                        arrayJSONAdapter = new ArrayAdapter(activity,  android.R.layout.simple_dropdown_item_1line, allProjectGroupsList);
                        groupMbrsAddAutoCompleteTextView.setAdapter(arrayJSONAdapter);
                        groupMbrsAddAutoCompleteTextView.setThreshold(1);
                        groupMbrsAddAutoCompleteTextView.setEnabled(true);
                    }
                    else if(checkedId==R.id.groupDefaultAddRadioBtnId)
                    {
                        isDefaultGrp = true;
            ...
            groupMbrsAddAutoCompleteTextView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
        ...
        addGroupMembersBtnLinearlayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(selectedMemberJArray.size()>0) {
                    String result = null;

                        result =  addMembersToProjectGroups(projectName, groupName, groupCreatedBy, selectedMemberJArray.toString(),isDefaultGrp);


                    if(result==null)
               {
                   Toast.makeText(activity,"Member(s) not added to Group",Toast.LENGTH_LONG).show();
               }
               else
               {
                   sendNotificationsToUsers();
                   googleJson = new Gson();
                   ArrayList<String> groupMemberArrayList =googleJson.fromJson(selectedMemberJArray.toString(), ArrayList.class);

                   zihronChatAppGroupChannel.InviteUsers(groupMemberArrayList,groupName);
                   Toast.makeText(activity,"Member(s)  added to Group",Toast.LENGTH_LONG).show();
               }
                }
                else
                {
                    Toast.makeText(activity, "No member selected to add to this group",Toast.LENGTH_LONG).show();
                }
                Intent pGACIntent = new Intent(activity,ProjectMembersGroupInviteActivity.class);
                 activity.startActivity(pGACIntent);
            }
        });
    }
...
public void setAutoCompleteTextViewDefault()
{
    ...
    groupCreatedBy = "admin";
    populateFriendsLayout();
    groupMbrsAddAutoCompleteTextView.setEnabled(false);
    groupMbrsAddAutoCompleteTextView.postDelayed(new Runnable() {
        @Override
        public void run() {
            groupMbrsAddAutoCompleteTextView.showDropDown();
        }
    },500);
    groupMbrsAddAutoCompleteTextView.setText(defaultGroupName);
    groupMbrsAddAutoCompleteTextView.setSelection(groupMbrsAddAutoCompleteTextView.getText().length());
}
...
public void populateFriendsLayout()
{
    try {
        String result =null;
        if(isDefaultGrp ) {
            result= getAllProjectDefaultGroupMembers(projectName);
        }
        else {
            result = getAllProjectGroupMembers(projectName, userName, groupName);
        }
        notGroupMemberJSONArray = new JSONArray(result);
        if (notGroupMemberJSONArray.length()>0) {
    ...
    public void createSelectedFriendHorizontalNormalView()
    {
        ...
        for(int i=0;i<notGroupMemberJSONArray.length(); i++) {
            try {
                tempArray = notGroupMemberJSONArray.getJSONArray(i);
           ...
           // friendNameTextView.setFilters(new InputFilter[]{new InputFilter.LengthFilter(10)});
            try {
                friendNameTextView.setText(tempArray.getString(1)+" "+tempArray.getString(2));
                ...
                selectBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                    ...
                    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                        ...
                        if(isChecked)
                        {
                            ...
                            addGroupMembersBtnLinearlayout.setEnabled(true);
                                selectedMemberJArray.add(notGroupMemberJSONArray.getJSONArray(index).getString(0));
         }
                        else
                        {

                            selectedMemberJArray.remove(notGroupMemberJSONArray.getJSONArray(index).getString(0));
                            if(selectedMemberJArray.size()==0)
                            {
                                addGroupMembersBtnLinearlayout.setEnabled(false);
                            }
                ...
                addFriendsToGrpLinearLayout.addView(innerSelectionLinearLayout);
            } catch (JSONException e) {
                e.printStackTrace();
        ...
        for(int i=0;i<notGroupMemberJSONArray.length(); i++) {
            try {
                ...
                if(s3ImageClass.confirmIfImageInPhone(tempArray.getString(0)))
                {
                    ...
                    {
                        Picasso.with(activity).load("https://ui-avatars.com/api/?name=" + tempArray.getString(0) + "&background=90a8a8&color=fff&size=128").into(profileImageVW);
                ...
                try {
                    ...
                    final CheckBox selectBox = new CheckBox(activity);
                    selectBox.setScaleX(1.0F);
                    selectBox.setScaleY(1.0F);
                    selectBox.setId(i);
                    RelativeLayout.LayoutParams rlpcloseImageVWLayout = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
                    rlpcloseImageVWLayout.addRule(RelativeLayout.RIGHT_OF, R.id.memberLogoId);
                    rlpcloseImageVWLayout.addRule(RelativeLayout.ALIGN_BOTTOM);
                    ...
                    RelativeLayout innerSelectionRelativeLayout = new RelativeLayout(activity);
                    RelativeLayout.LayoutParams innerSelectionRelativeLayoutLP = new RelativeLayout.LayoutParams(250, LinearLayout.LayoutParams.WRAP_CONTENT);
                    innerSelectionRelativeLayout.setLayoutParams(innerSelectionRelativeLayoutLP);
                    innerSelectionRelativeLayout.addView(profileImageVW);
                    innerSelectionRelativeLayout.addView(selectBox);
    ...
    public void sendNotificationsToUsers()
    {
        ...
        String result =null;
       try {
            result =  createNotifications(userName, selectedMemberJArray.toString(),  "02","Requested",groupName,projectName);
    ...
    public String getAllProjectDefaultGroupMembers(String projectName)
    {
        String  result = null;
        JSONObject postDataParams = new JSONObject();
        try {
            ...
            postDataParams.put("nameOfUser", userName);
                ...
                httpRequestClass = new HttpRequestClass("http://naijaconnectsapis.ca/projectmanagement-1.0/projectmanagement/fanpgm/",postDataParams, activity,"application/json", "application/json");
    ...
    public String getAllProjectGroupMembers(String projectName, String userName,String groupName)
    {
        String  result = null;
        JSONObject postDataParams = new JSONObject();
        try {
            ...
            postDataParams.put("nameOfGroup", groupName);
                ...
                httpRequestClass = new HttpRequestClass("http://naijaconnectsapis.ca/projectmanagement-1.0/projectmanagement/fanpm/",postDataParams, activity,"application/json", "application/json");
    ...
    public String getAllProjectGroups(String projectName, String groupName)
    {
        String  result = null;
        JSONObject postDataParams = new JSONObject();
        try {
    ...
    public String addMembersToProjectGroups(String projectName, String groupName,String groupCreatedBy, String allMembersToString,boolean isDefaultGroup)
    {
        String  result = null;
        JSONObject postDataParams = new JSONObject();
        try {
            postDataParams.put("nameOfProject", projectName);
            postDataParams.put("nameOfGroup", groupName);
            postDataParams.put("isDefaultGroup", isDefaultGroup);
            postDataParams.put("groupCreatedBy", groupCreatedBy);
            postDataParams.put("allMembersToString", allMembersToString);


            httpRequestClass = new HttpRequestClass("http://naijaconnectsapis.ca/projectmanagement-1.0/projectmanagement/ipgm/",postDataParams, activity,"text/plain", "application/json");
            result = httpRequestClass.getResult();
            asyncErrorDialogDisplay = new AsyncErrorDialogDisplay(activity);
            if(result!=null) {
                if (result.equalsIgnoreCase("exception")) {
